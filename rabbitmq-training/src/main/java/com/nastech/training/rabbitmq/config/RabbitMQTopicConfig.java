...
public class RabbitMQTopicConfig {
    ...
    @Bean
    Binding electronicBinding (Queue queueName, TopicExchange topicExchange) {
        return BindingBuilder.bind(queueName).to(topicExchange).with(electronicKey);
    }

    @Bean
    Binding fashionBinding (Queue queueName, TopicExchange topicExchange) {
        return BindingBuilder.bind(queueName).to(topicExchange).with(fashionKey);
    }

    @Bean
    Binding kidFashionBinding (Queue queueName, TopicExchange topicExchange) {
        return BindingBuilder.bind(queueName).to(topicExchange).with(kidFashionKey);
    }

    @Bean
    Binding doyBinding (Queue queueName, TopicExchange topicExchange) {
        return BindingBuilder.bind(queueName).to(topicExchange).with(doyKey);
    }
