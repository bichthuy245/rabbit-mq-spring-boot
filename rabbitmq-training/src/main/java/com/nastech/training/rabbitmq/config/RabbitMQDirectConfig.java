...
public class RabbitMQDirectConfig {
    ...
    Binding electronicBinding (Queue electronicQueue, DirectExchange exchange) {
        return BindingBuilder.bind(electronicQueue).to(exchange).with(electronicKey);
    }

    @Bean
    Binding fashionBinding (Queue fashionQueue, DirectExchange exchange) {
        return BindingBuilder.bind(fashionQueue).to(exchange).with(fashionKey);
    }

    @Bean
    Binding kidFashionBinding (Queue queueName, DirectExchange exchange) {
        return BindingBuilder.bind(queueName).to(exchange).with(kidFashionKey);
    }

    @Bean
    Binding doyBinding (Queue queueName, DirectExchange exchange) {
        return BindingBuilder.bind(queueName).to(exchange).with(doyKey);
    }
